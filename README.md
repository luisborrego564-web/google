# Zplix — Login con auto-registro (Flask + PostgreSQL)
...
datos PostgreSQL. Si el correo no existe, la cuenta se crea automáticamente
...
database.py    # Conexión PostgreSQL y consultas SQL parametrizadas
...
El proyecto usa un `DATABASE_URL` de un Postgres gestionado (recomendado:
[neon.tech](https://neon.tech), plan gratuito permanente) para que los datos
sobrevivan a reinicios/redeploys, a diferencia de un archivo SQLite local en
hosting gratuito (que se pierde en cada redeploy).

1. Crea una cuenta gratis en neon.tech y un proyecto.
2. Copia el "Connection string" (`postgresql://usuario:password@host/db?sslmode=require`).
...
4. Agrega manualmente la variable de entorno `DATABASE_URL` (tu connection
   string de Neon) en Render → Settings → Environment.
